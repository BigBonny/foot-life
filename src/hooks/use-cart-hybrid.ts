  ...
  persist(
    ...
    }),
    {
      ...
      onRehydrateStorage: (state) => {
        console.log('Cart rehydrated from storage:', state)
        // When cart is rehydrated, check if we need to load from database
        // This happens when user comes back to the site
        if (typeof window !== 'undefined') {
          const userId = 'user_3BAWl1Nle0eGAtblJQzPKvVSHhG' // TODO: Get actual user ID
          if (userId && state.items?.length === 0) {
            console.log('Loading cart from database on rehydrate')
            // Load from database
            const store = useCart.getState()
            store.loadFromDatabase(userId)
          }
        }
      },
    }
